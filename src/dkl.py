...
from gpytorch.constraints import LessThan, Positive, Interval


class ConstantMeanBounded(Mean):
    def __init__(self, prior=None, batch_shape=torch.Size(), **kwargs):
        super(ConstantMeanBounded, self).__init__()
    ...
    def forward(self, input):
        if input.shape[:-2] == self.batch_shape:
            return self.sigmoid(self.constant.expand(input.shape[:-1]))
        else:
            return self.sigmoid(self.constant.expand(_mul_broadcast_shape(input.shape[:-1], self.constant.shape)))
...
class GP(ApproximateGP):
    def __init__(
        ...
        kwargs = {
            ...
            # "lengthscale_constraint": Interval(lower_bound=0.0, upper_bound=1.0, transform=torch.nn.Softplus()),
        }
        ...
        self.mean_module = ConstantMeanBounded(batch_shape=batch_shape, )
        from gpytorch.priors import NormalPrior
        self.covar_module = ScaleKernel(kernel, batch_shape=batch_shape,
                                        outputscale_constraint=LessThan(upper_bound=1.0, transform=torch.nn.Softplus()))
